...
function openHtml(terms) {
    ...
    for (var i = 0; i < terms.length; i++) {
        html += '<div><a class="link category ' + terms[i] + '" href="' + terms[i] + '">' + terms[i] + '</a></div>';
    }
    html += '</div>';
    return html;
...
function getStyle() {
    return `<script src="//cdnjs.cloudflare.com/ajax/libs/list.js/1.5.0/list.min.js"></script><style>
				...
				.heading{color:black !important; z-index:2;padding:0px; width:100%;text-align:center;font-size:19px;}
				...
				.item{width:100%; clear:both;padding-bottom:10px; margin-bottom:10px; }
				.selected{color:#000 !important;}
				h2{color:#fff;background:#093145;}
				.item img{width:100%;max-width:275px; float:left; margin-right: 10px; }
